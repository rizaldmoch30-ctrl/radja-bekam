...
export default function AdminServicesPage() {
  ...
  const openEditModal = (s: Service) => {
    setFormData({
      ...
      category: s.category || "Terapi Bekam",
      isActive: s.isActive,
    });
    setEditingId(s.id);
    setIsModalOpen(true);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <PageHeader 
          ...
          rightContent={
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200 active:scale-95"
            >
        ...
        <div className="space-y-4">
          {CATEGORIES.map((cat, catIndex) => {
            const filteredServices = services.filter(s => s.category === cat || (!s.category && cat === "Terapi Bekam"));
            const isOpen = openCategory === cat;
            
            return (
              <div key={cat} className={`bg-white rounded-2xl border transition-all duration-300 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 ${isOpen ? 'border-indigo-200' : 'border-gray-100 hover:border-gray-200'}`} style={{ animationDelay: `${catIndex * 100}ms` }}>
                <button 
                  onClick={() => setOpenCategory(isOpen ? null : cat)}
                  className={`w-full px-6 py-4 flex items-center justify-between transition-colors ${isOpen ? 'bg-indigo-50/80' : 'bg-white hover:bg-gray-50'}`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${isOpen ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 text-gray-600'}`}>
                  ...
                  <div className={`p-2 rounded-full transition-colors ${isOpen ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
                    {isOpen ? <ChevronDown className="w-5 h-5" /> : <ChevronRight className="w-5 h-5" />}
                  </div>
                </button>
                
                {/* Accordion Content */}
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[2000px] opacity-100 border-t border-indigo-100' : 'max-h-0 opacity-0'}`}>
            ...
            <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
              ...
              <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
                ...
                <button
                  ...
                  form="serviceForm"
                  disabled={saving}
                  className="px-5 py-2.5 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 disabled:opacity-50 flex items-center"
                >
